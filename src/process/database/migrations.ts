...
/**
 * Migration v14 -> v15: Remove Gemini agent type
 * Convert existing gemini conversations to acp and update CHECK constraints
 */
const migration_v15: IMigration = {
  version: 15,
  name: 'Remove Gemini agent type',
  up: (db) => {
    // Convert existing gemini conversations to acp
    db.exec(`
      UPDATE conversations SET type = 'acp' WHERE type = 'gemini';
    `);

    // Recreate conversations table without gemini in CHECK constraint
    db.exec(`
      CREATE TABLE IF NOT EXISTS conversations_v15 (
        id TEXT PRIMARY KEY,
        user_id TEXT NOT NULL,
        name TEXT NOT NULL,
      ...
      INSERT INTO conversations_v15 (id, user_id, name, type, extra, model, status, source, channel_chat_id, created_at, updated_at)
      SELECT id, user_id, name, type, extra, model, status, source, channel_chat_id, created_at, updated_at FROM conversations;

      DROP TABLE conversations;
      ALTER TABLE conversations_v15 RENAME TO conversations;
    ...
    console.log('[Migration v15] Removed Gemini agent type, Converted gemini conversations to acp.');
  },
  down: (db) => {
    // Rollback: Add gemini back to CHECK constraint
    db.exec(`
      CREATE TABLE IF NOT EXISTS conversations_rollback (
        id TEXT PRIMARY KEY,
        user_id TEXT NOT NULL,
    ...
    console.log('[Migration v15] Rolled back: Restored Gemini agent type');
  },
};
