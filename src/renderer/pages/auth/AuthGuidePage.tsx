      ...
      <div className='auth-guide-card'>
        ...
        <div className='auth-guide-form'>
          <div className='auth-guide-input-wrapper'>
            <Input prefix={<IconLock />} placeholder='请输入授权码' value={authCode} onChange={setAuthCode} onKeyDown={handleKeyDown} size='large' disabled={isLoading} className='auth-guide-input' />
          </div>

          <Button type='primary' size='large' long loading={isLoading} disabled={!authCode.trim()} onClick={handleSubmit} className='auth-guide-submit'>
            确认
          </Button>
