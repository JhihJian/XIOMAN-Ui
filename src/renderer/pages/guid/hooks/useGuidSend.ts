...
export const useGuidSend = (deps: GuidSendDeps): GuidSendResult => {
  const { input, setInput, files, setFiles, dir, setDir, setLoading, selectedAgent, selectedAgentKey, selectedAgentInfo, isPresetAgent, selectedMode, selectedAcpModel, currentModel, findAgentByKey, resolvePresetRulesAndSkills, isGoogleAuth, setMentionOpen, setMentionQuery, setMentionSelectorOpen, setMentionActiveIndex, navigate, closeAllTabs, openTab, t } = deps;

  const handleSend = useCallback(
    async (messageOverride?: string) => {
      // Use messageOverride if provided, otherwise use input state
      ...
      if (!selectedAgent || selectedAgent === 'gemini') {
        ...
        try {
          ...
          const displayMessage = buildDisplayMessage(messageToSend, files, workspacePath);
      ...
      // ACP path - always use claude as backend
      {
        const acpBackend: AcpBackend = 'claude';
    ...
    [input, files, dir, selectedAgent, selectedAgentKey, selectedAgentInfo, isPresetAgent, selectedMode, selectedAcpModel, currentModel, findAgentByKey, resolvePresetRulesAndSkills, navigate, closeAllTabs, openTab]
  );
  ...
  const isButtonDisabled = !input.trim() || ((!selectedAgent || selectedAgent === 'gemini') && !isPresetAgent && !currentModel && isGoogleAuth);
